class NPC extends engine.DisplayObjectContainer implements Observer{
    ...
    constructor(id:string,tex:string){
        ...
        if(flag==Task.ACCEPTABLE){
            this.emoji.texture=engine.Resourse.getInstance().getRes("tanhao_yellow.png");
        }else if(flag==Task.CAN_SUBMIT){
            this.emoji.texture=engine.Resourse.getInstance().getRes("wenhao_yellow.png");
        }else if(flag==Task.DURING){
            this.emoji.texture=engine.Resourse.getInstance().getRes("wenhao_gray.png");
        }else{
